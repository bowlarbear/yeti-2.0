...
It's certainly not a bad idea for you to do this, but its superfluous for the purposes of the main guide and the warning is irrelevant to our main concerns, which is that the binary we have downloaded from bitcoincore.org is what it claims to be, and that it has been signed (we assume reproduced) by a number of Bitcoin Core contributors.
